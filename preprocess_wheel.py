...
def preprocess_wheel(alldat, verbose = False):
    s = 0
    ...
    stim_vel_trial = np.array([])
    stim_accel_trial = np.array([])
    goc_vel_trial = np.array([])
    goc_accel_trial = np.array([])
    rt_vel_trial = np.array([])
    rt_accel_trial = np.array([])
    ...
    for dat in alldat:
        ...
        for trial in range(len(dat)): 
            ...
            goc = int(np.round((dat['gocue'][trial]+500)/10))
            stim = int(np.round(500/10))
            rt = int(np.round((dat['response_time'][trial]+500)/10))
            
            stim_vel_trial = np.concatenate([stim_vel_trial, wheel_velocity[trial][stim-1:stim+3]])
            stim_accel_trial = np.concatenate([stim_accel_trial, wheel_acceleration[trial][stim-1:stim+3]])
            goc_vel_trial = np.concatenate([goc_vel_trial, wheel_velocity[trial][goc-1:goc+3]])
            goc_accel_trial = np.concatenate([goc_accel_trial, wheel_acceleration[trial][goc-1:goc+3]])
            rt_vel_trial = np.concatenate([rt_vel_trial, wheel_velocity[trial][rt-4:rt+2]])
            rt_accel_trial = np.concatenate([rt_accel_trial, wheel_acceleration[trial][rt-4:rt+2]])
    ...
    my_dict = {'session': session, 
        ...
        'stim_vel_trial': stim_vel_trial,
        'stim_accel_trial': stim_accel_trial,
        'goc_vel_trial': goc_vel_trial,
        'goc_accel_trial': goc_accel_trial,
        'rt_vel_trial': rt_vel_trial,
        'rt_accel_trial': rt_accel_trial
              }
